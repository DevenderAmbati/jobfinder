...
describe('Lever provider pipeline integration', () => {
  ...
  beforeAll(async () => {
    ...
    const company = await prisma.company.create({
      data: {
        name: `Lever Test ${Date.now()}`,
        provider: 'lever',
        careerUrl: 'https://jobs.lever.co/pipeline-lever',
        enabled: true,
        frequency: '0 0 1 1 *',
      },
    });
  ...
  it('runs fetch → normalize → dedup → rules → AI → notify → persist', async () => {
          ...
          JSON.stringify([
            {
              ...
              applyUrl: 'https://jobs.lever.co/pipeline-lever/1/apply',
              categories: { location: 'Hyderabad, India', team: 'Engineering' },
              descriptionPlain: 'TypeScript React Node.js services',
              createdAt: Date.now(),
            },
            {
              id: '2',
              text: 'Engineering Manager',
              applyUrl: 'https://jobs.lever.co/pipeline-lever/2/apply',
              categories: { location: 'Bangalore, India' },
              descriptionPlain: 'Lead people',
            },
            {
              id: '3',
              text: 'Backend Engineer',
              applyUrl: 'https://jobs.lever.co/pipeline-lever/3/apply',
              categories: { location: 'Remote, India', department: 'Platform' },
              descriptionPlain: 'Node.js and TypeScript APIs',
            },
          ]),
    ...
    const useCase = new FetchCompanyJobsUseCase({
      ...
      providers: new ProviderRegistry([
        new LeverProvider({ fetchImpl: fetchImpl as typeof fetch }),
      ]),
      notifier,
    ...
    expect(persisted.every((job) => job.provider === 'lever')).toBe(true);

    // Mocked resume score 88 at 60% plus perfect rule fit (100) at 40%.
    ...
    const engineerMatch = engineer
      ...
      : null;
    expect(engineerMatch?.matchScore).toBe(93);
