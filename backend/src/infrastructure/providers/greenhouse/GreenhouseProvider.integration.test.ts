...
class MockMatcher implements JobMatcher {
  async match(_resumeText: string, _job: Job): Promise<MatchResult> {
    return {
      score: 90,
      reasons: ['Mocked AI match'],
      missingSkills: [],
      interviewDifficulty: 'Medium',
      salaryEstimate: '30 LPA',
...
describe('Greenhouse provider pipeline integration', () => {
  ...
  beforeAll(async () => {
    ...
    const company = await prisma.company.create({
      data: {
        name: `Greenhouse Test ${Date.now()}`,
        provider: 'greenhouse',
        careerUrl: 'https://boards.greenhouse.io/pipeline-test',
        enabled: true,
        frequency: '0 0 1 1 *',
      },
    });
  ...
  it('runs fetch → normalize → dedup → rules → AI → notify → persist', async () => {
          ...
          jobs: [
            {
              ...
              absolute_url: 'https://boards.greenhouse.io/pipeline-test/jobs/101',
              location: { name: 'Hyderabad, India' },
              content:
                '<p>Work with TypeScript, React, and Node.js on product APIs.</p>',
              departments: [{ name: 'Engineering' }],
              first_published: '2024-06-01T00:00:00.000Z',
            },
            {
              id: 102,
              title: 'Engineering Manager',
              absolute_url: 'https://boards.greenhouse.io/pipeline-test/jobs/102',
              location: { name: 'Bangalore, India' },
              content: '<p>Manage engineers</p>',
            },
            {
              id: 103,
              title: 'Backend Engineer',
              absolute_url: 'https://boards.greenhouse.io/pipeline-test/jobs/103',
              location: { name: 'Remote, India' },
              content: '<p>Node.js services and TypeScript</p>',
            },
          ],
        }),
    ...
    const useCase = new FetchCompanyJobsUseCase({
      ...
      providers: new ProviderRegistry([
        new GreenhouseProvider({ fetchImpl: fetchImpl as typeof fetch }),
      ]),
      notifier,
    ...
    expect(
      persisted.every((job) => job.provider === 'greenhouse'),
    ).toBe(true);

    // Mocked resume score 90 at 60% plus perfect rule fit (100) at 40%.
    ...
    const engineerMatch = engineer
      ...
      : null;
    expect(engineerMatch?.matchScore).toBe(94);
    ...
    const notifications = await prisma.notificationLog.count({
      where: { jobId: { in: persisted.map((job) => job.id) }, success: true },
    });
